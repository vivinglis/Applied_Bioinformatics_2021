...
for (corr in corrs){
  ...
  #   corr_veg <- data.frame(GO_term = top_corr, BA_veg = pearson_corr_BA_veg[top_corr, "Value"], sa_veg = pearson_corr_sa_veg[top_corr, "Value"], sa_sex = pearson_corr_sa_sex[top_corr, "Value"])
  ...
  #   corr_veg <- data.frame(GO_term = top_corr, BA_veg = pearson_corr_BA_veg[top_corr, "Value"], sa_veg = pearson_corr_sa_veg[top_corr, "Value"], BA_sex = pearson_corr_BA_sex[top_corr, "Value"])
  #   top_veg <- rbind(top_veg, corr_veg)
  # }
  ...
  #   corr_veg <- data.frame(GO_term = top_corr, sa_sex = pearson_corr_sa_sex[top_corr, "Value"], sa_veg = pearson_corr_sa_veg[top_corr, "Value"], BA_sex = pearson_corr_BA_sex[top_corr, "Value"])
  #   top_veg <- rbind(top_veg, corr_veg)
  # }
  # 
  ...
  for (top_corr in top_pearson_corr_sa_veg$GO_term){
    corr_veg <- data.frame(GO_term = top_corr, sa_sex = pearson_corr_sa_sex[top_corr, "Value"], BA_veg = pearson_corr_BA_veg[top_corr, "Value"], BA_sex = pearson_corr_BA_sex[top_corr, "Value"])
    top_veg <- rbind(top_veg, corr_veg)
  }
  
  #Mean correlation values
  #avg_tot <- mean(corr$Value, na.rm=TRUE)
  # avg_sex <- mean(top_pearson_corr_sex$Value, na.rm=TRUE)
  # avg_veg <- mean(top_pearson_corr_veg$Value, na.rm=TRUE)
  # avg_BA_sex <- mean(top_pearson_corr_BA_sex$Value, na.rm=TRUE)
  # avg_sa_sex <- mean(top_pearson_corr_sa_sex$Value, na.rm=TRUE)
  # avg_BA_veg <- mean(top_pearson_corr_BA_veg$Value, na.rm=TRUE)
  # avg_sa_veg <- mean(top_pearson_corr_sa_veg$Value, na.rm=TRUE)
  ...
  # Plotting choose one of these for top 100 of that sample 
  #a <- list(top_pearson_corr_BA_sex$Value, top_veg$sa_sex, top_veg$BA_veg, top_veg$sa_veg ) #BA_sex
  #a <- list(top_veg$BA_sex, top_pearson_corr_sa_sex$Value, top_veg$BA_veg, top_veg$sa_veg ) #sa_sex
  #a <- list(top_veg$BA_sex, top_veg$sa_sex, top_pearson_corr_BA_veg$Value, top_veg$sa_veg ) #BA_veg
  a <- list(top_veg$BA_sex, top_veg$sa_sex, top_veg$BA_veg, top_pearson_corr_sa_veg$Value ) #sa_veg
...
title <- ggdraw() + 
  draw_label(
    "Correlation values for top 100 correlated GO terms for sa_veg",
    fontface = 'bold',
    x = 0,
    hjust = 0
  ) +
